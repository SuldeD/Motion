...
const state = proxy({
  clicked: null,
  urls: [1, 2, 3, 4, 5, 6, 7, 1, 5, 7, 2, 4, 6].map(
    (u) => `/carousel/${u}.webp`
  ),
});
    ...
    <group ref={ref}>
      {urls.map((_, i) => (
        <line
          ...
          geometry={geometry}
          material={material}
          position={[i * 0.06 - urls.length * 0.03, -height / 2 + 0.9, 0]}
        />
      ))}
    </group>
  );
...
export default function Page() {
  return (
    <div className="flex flex-col items-center justify-center h-[90svh] overscroll-none">
